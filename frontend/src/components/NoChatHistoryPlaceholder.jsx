...
const NoChatHistoryPlaceholder = ({ name, setInputText }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-6">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        <button
          onClick={() => setInputText && setInputText("👋 Say Hello")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors"
        >
          👋 Say Hello
        </button>
        <button
          onClick={() => setInputText && setInputText("🤝 How are you?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors"
        >
          🤝 How are you?
        </button>
        <button
          onClick={() => setInputText && setInputText("📅 Meet up soon?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors"
        >
          📅 Meet up soon?
        </button>
